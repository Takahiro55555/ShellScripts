...
# WSL をインストールした際に最初に実行するスクリプト（個人用）
# Ubuntu18.04で動作確認
...
if [ ! -e $vimrc_path ]; then
  ...
  echo "'.vimrc' already exsisting on \"$vimrc_path\""
  echo "Do you add vim settings on \"$vimrc_path\"?"
  # Yes No を聞く
  while :
    do  
  ...
  if [ "$ans" = "y" ] || [ "$ans" = "Y" ]; then
    ...
    echo "R:'.vimrc' set up was cancelled"
  fi
fi